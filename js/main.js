 ...
 * @pre boardLength and boardHeight have been changed by the user and both are valid inputs.
 * @post minesInput's readOnly attribute has been set to true or false.
 */
 ...
 * @pre startButton has been clicked by the user and boardLength, boardHeight, and minesInput are valid inputs.
 * @post game is now a Board with mines and adjacent numbers planted, game is represented as a table, startButton is now red and reads Restart, and flags/mines stat bar is now shown.
 */
 ...
 * @pre A tile on the game board has been left-clicked by the user, the game is still running, the tile clicked on has not yet been revealed
 * @post The tile clicked on has been revealed, as well as any adjacent tiles if the tile clicked on is a 0
 ...
 * @param {Object} clicked The TD object that was clicked, gotten from event.target
 * @pre A tile on the game board has been right-clicked by the user, the game is still running, the tile clicked on has not yet been revealed
 * @post The tile clicked on has been flagged if not already, the flag is removed if it was already flagged.
 * @return {null} Return null only if the game is over or the tile has already been revealed.
 */
